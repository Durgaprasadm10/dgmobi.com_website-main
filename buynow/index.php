...
if (Config::CURRENT_ENV == Config::PROD_ENV) { //PRODUCTION
  ...
  $paypalReturnURL = Config::PAYPAL_RETURN_URL;
     //$paypalReturnURL = "http://test.dgmobi.com/buynow/success.php";
...
<script>
  var consoleDebug=true  //for test site for production false 
...
function updatePriceDisplay(productid, selected,typeUpdation="") {
    if(consoleDebug){
        console.log(productid);
    }
    console.log(typeUpdation);
   ...
   // price = +productsList11[productid].annual_price;
                            ...
                            function validatepassword(pass){
                                if(pass!=""){///^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/;
                                  // var filter= /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/;
                                    var filter= /^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/;  //8 letter password, with at least a symbol, upper and lower case letters and a number
                                    if(!pass.match(filter)){
                                        alert("8 letter password, with at least a symbol, upper and lower case letters and a number");
                                              $("#password").val("");
                                    }
                                }
                            ...
                            function registration(){
                                ...
                                bootbox.dialog({
                                    ...
                                    <span>First Name</span>
                                     ...
                                     <input type="email" id="emailId" placeholder="Email" onblur=validateEmail(this.value)>
                                     <span style=" padding-left: 50px;">Phone number</span>
                                     <input type="number" id="phoneNumber" placeholder="Phone Number" pattern="[0-9]{3}-[0-9]{3}-[0-9]{4}">
                                     </br></br>
                                     <span>User Name</span>
                                     <input type="text" id="username" placeholder="User Name">  
                                     <span style=" padding-left: 78px;">Password</span>
                                     <input type="password" id="password" placeholder="Password"  onblur=validatepassword(this.value)>
                                      </body>`,// "<body>" + popup_body_container + "</body>",   onblur=fncheckdetails(this.value)
...
formDetails.email=$("#emailId").val();
formDetails.firstname=$("#firstName").val();
formDetails.lastname=$("#lastName").val();
formDetails.phone=$("#phoneNumber").val();
...
function fncheckdetails(value) {
				...
				$.ajax({
					...
					if(resp.avail == "true") {
						...
						$("#lastName").val(resp.lastName);//lastName
						$("#phoneNumber").val(resp.phoneNumber);
						$("#emailId").val(resp.emailId);
		...
		<?php }?>
            ...
            <!-- <center> -->
                                    ...
                                    <div class="text-center col col-6" style=" width: 55%; padding-top: 10px;">
	                                  <input type=radio class="radio_button"  name=lic_type value=new onchange=updatePriceDisplay(product_select_id.options[product_select_id.selectedIndex].value,0,"typeUpdation") checked>
                                      <span style="font-size: 18px; font-weight: bold;" id="newlicense">Annual License</span> &nbsp;&nbsp;
                                      <input type=radio class="radio_button"  onchange=updatePriceDisplay(product_select_id.options[product_select_id.selectedIndex].value,1,"typeUpdation") name=lic_type value=life>
